...
{
    public partial class InicialView : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            bttPostar.Attributes.Add("Style", "Border-Color: #a800ff");
            txtPostagem.Attributes.Add("Style", "Border-Color: #a800ff");
        }
        public void CriaTimeline()
        {
            ...
            List<followusuario> listafollow = FUR.GetAllById(UsuarioAtual.id_usuario);
            for (int lf = 0; lf < listafollow.Count; lf++)
            {
                usuario UsuarioPostador = UR.GetOne(listafollow[lf].usuario_id_usuario);
                if (UsuarioPostador != null)
                {
                    List<post> listapostfollow = PR.GetAllById(UsuarioPostador.id_usuario);
                    for (int lpf = 0; lpf < listapostfollow.Count; lpf++)
                    {
                        DataRow datarow = table.NewRow();
                        datarow["TempoPostagem"] = DateTime.Now.ToBinary() - listapostfollow[lpf].datapostagem.ToBinary();
                        datarow["DataPostagem"] = listapostfollow[lpf].datapostagem.ToString("dd/MM/yyyy HH:mm");
                        datarow["id_usuario"] = UsuarioPostador.id_usuario;
                        datarow["id_post"] = listapostfollow[lpf].id_post;
                        datarow["ConteudoPost"] = listapostfollow[lpf].contentpost;
                        datarow["NomeUsuario"] = UsuarioPostador.nomeusuario;
                        datarow["NicknameUsuario"] = UsuarioPostador.nicknameusuario;
                        datarow["LinkfotoUsuario"] = UsuarioPostador.linkfoto;
                        table.Rows.Add(datarow);
                    }
                }
            }
            List<post> lista = PR.GetAllById(UsuarioAtual.id_usuario);
            for (int i = 0; i < lista.Count; i++)
            {
                ...
                datarow["id_post"] = lista[i].id_post;
                datarow["ConteudoPost"] = lista[i].contentpost;
                datarow["NomeUsuario"] = UsuarioAtual.nomeusuario;
                datarow["NicknameUsuario"] = UsuarioAtual.nicknameusuario;
                datarow["LinkfotoUsuario"] = UsuarioAtual.linkfoto;
                table.Rows.Add(datarow);
            }
            if (table.Rows.Count == 0)
            {
                ...
                datarow["id_post"] = 0;
                datarow["ConteudoPost"] = "Tudo vazio por aqui! Poste algo! Siga Perfil's";
        ...
        protected void bttPostar_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(txtPostagem.Text))
            {
                bttPostar.Attributes.Add("Style", "Border-Color: Red");
                txtPostagem.Attributes.Add("Style", "Border-Color: Red");
            }
            else
            {
                post Post = new post();
                Post.contentpost = txtPostagem.Text;
                Post.id_usuario = UsuarioAtual.id_usuario;
        ...
        protected void rptComentario_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            post Postagem = PR.GetOne((int)Session["idpostaberto"]);
            if (Link != null || deleta != null)
                if (int.Parse(Link.CommandArgument) == UsuarioAtual.id_usuario || Postagem.id_usuario == UsuarioAtual.id_usuario)
                {
                    deleta.Visible = true;
                    deleta.Enabled = true;
                }
        ...
        protected void rptPostagem_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            LinkButton Link = (LinkButton)e.Item.FindControl("lnkUsuarioPostagem");
            if (Link != null || deleta != null)
            ...
            Repeater rptComentarioPost = (Repeater)e.Item.FindControl("rptComentarioPost");
            ...
            List<comentario> listacomentario = CR.GetAllById(int.Parse(deleta.CommandArgument));
            ...
            rptComentarioPost.DataSource = dv.ToTable();
            rptComentarioPost.DataBind();
        }

        protected void Escolha_Command(object sender, CommandEventArgs e)
        {
            Session["aviso"] = bool.Parse(e.CommandArgument.ToString());
            lblMensagemShow.Text = "#mensagem";
            pnlConfirmacao.Attributes.Add("style", "Display: none");

            imgExcluiPost_Command(null, null);
        }

        public void Show(string aviso)
        {
            Session.Remove("resposta");
            pnlConfirmacao.Attributes.Add("style", "Display: inline-grid");
            lblMensagemShow.Text = lblMensagemShow.Text.Replace("#mensagem", aviso);
        }
        ...
        protected void rptComentarioPost_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ImageButton deleta = (ImageButton)e.Item.FindControl("imgExcluiComentario");
            LinkButton Link = (LinkButton)e.Item.FindControl("lnkUsuarioComentario");

            var itemPai = e.Item.Parent.Parent;
            ImageButton idimgDelete = (ImageButton)itemPai.FindControl("imgExcluiPost");

            post Postagem = PR.GetOne(int.Parse(idimgDelete.CommandArgument));
